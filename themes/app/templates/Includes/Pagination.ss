<% if $MoreThanOnePage %>
    <div class="d-flex flex-row flex-wrap align-items-center pagination">
        <a class="pagination__link <% if not $NotFirstPage %>disabled<% end_if %>" href="$baseURL$PrevLink">
        ...
        <div class="pagination__mobile">
            <% loop $PaginationSummary(1) %>
                <% if $CurrentBool %>
                    <a class="pagination__link" href="$baseURL$Link" title="View page {$PageNum}">
        ...
        <a class="pagination__link <% if not $NotLastPage %>disabled<% end_if %>" href="$baseURL$NextLink">
